...
import type { ConfigCategoria, ConfigCampoExtra } from '@/lib/types';
import { Plus, Trash2, Settings2, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Constructor de Formularios</h1>
          <p className="text-slate-500 font-medium">Define campos personalizados por cada sector operativo</p>
        </div>
      </div>
          ...
          {categorias.map((cat) => (
            ...
            >
              <div 
                ...
                style={{ backgroundColor: selectedCatId === cat.id ? '#fff' : (cat.color || '#000') }}
              />
              {cat.nombre}
            </button>
          ))}
            ...
            <div className="card p-6 space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-900">Campos para {selectedCat.nombre}</h3>
              ...
              <div className="space-y-3">
                {selectedCat.camposExtra && selectedCat.camposExtra.length > 0 ? (
                  ...
                  <div className="py-20 text-center space-y-3">
                    ...
                    <p className="text-sm text-slate-400 font-medium">No hay campos personalizados aún.<br/>Añade uno para empezar a recolectar datos específicos.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500">Configura un nuevo input para la categoría {selectedCat?.nombre}</p>
            </div>
            <form onSubmit={handleAddField} className="p-6 space-y-5">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nombre del Campo</label>
                <input
                  required
                  type="text"
              ...
              <label className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl cursor-pointer border border-slate-100 hover:border-primary-200 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium">El ciudadano no podrá enviar el reporte sin completar este campo</p>
                </div>
              </label>
